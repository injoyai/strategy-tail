...
```text
完整成交：单笔净收益率 = (SellIncome - BuyCost) / BuyCost
旧记录缺少成本字段：单笔收益率 = (SellPrice - BuyPrice) / BuyPrice
ProfitFactor = Σ盈利单收益率 / |Σ亏损单收益率|
```

原因：本系统按"一手"等数量买入，金额口径会被高价股亏损放大，导致盈亏比失真。
百分比口径下高低价股权重一致；完整回测进一步扣除滑点、佣金、印花税等成本，
结果反映可实现的净策略质量。仅为兼容缺少 `BuyCost/SellIncome` 的旧记录时回退毛收益。

### 计算规则

* 单笔盈亏取**净收益率（%）**，不是绝对金额；旧记录按上述规则回退。
...
* **一个方法只做一件事**：计算、副作用、IO 尽量分离。计算型函数返回结果，由调用方决定如何使用，不要把多件事塞进同一个函数。

* **不要用指针参数制造隐式副作用**：需要返回数据就 `return`，由调用方 `append`/赋值。禁止把 `*[]T` 之类传进去让函数偷偷 append。
  反例：`executeSell(ts *[]Trade)` 内部 append → 已改为 `executeSell(...) Trade` 返回，调用方 `ts = append(ts, this.executeSell(...))`。

* **不用的方法接收者改为普通函数**：`func (this T) foo()` 里若没用到 `this`，应写成 `func foo()`。

* 遵循 Go 惯用写法：`gofmt` 格式化、错误即时 `if err != nil` 处理、命名清晰、避免包级全局可变状态。
...
### 不要整理 `cmd/backtest/main.go`

* **不要轻易整理、重构、删除** [cmd/backtest/main.go](cmd/backtest/main.go) 中的代码。该文件是快速试验入口，作者会保留大量临时/注释/无效代码用于对比测试，这些"看起来没用的代码"是有意保留的。

* 仅当作者明确要求修改该文件时才动它，不要顺手清理。
...
* [ ] 代码符合规范：一个方法只做一件事、无指针参数隐式副作用（该 return 的就 return）、无用接收者已改为普通函数、`gofmt` 通过。
