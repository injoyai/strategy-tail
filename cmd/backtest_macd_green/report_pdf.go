...
// ExportPDF 导出 PDF 报告到 output/backtest-macd-green/report.pdf
func ExportPDF(r *ReportData) error {
	pdf := &gopdf.GoPdf{}
	...
	y += 2
	y = drawAdvice(pdf, y, r)

	// ===== 写文件 =====
	dir := filepath.Join("output", "backtest-macd-green")
...
func drawHeader(pdf *gopdf.GoPdf, r *ReportData, y float64) float64 {
	...
	pdf.SetX(pdfMarginX + 8)
	pdf.SetY(y + 23)
	pdf.Cell(nil, fmt.Sprintf("基准: %s ｜ 年份: %v ｜ 生成日期: %s",
		r.Benchmark, r.Years, r.GeneratedAt))
	return y + 32
}

// drawStrategyInfo 策略说明卡片
func drawStrategyInfo(pdf *gopdf.GoPdf, r *ReportData, y float64) float64 {
...
func drawYearlyTable(pdf *gopdf.GoPdf, y float64, r *ReportData) float64 {
	...
	for i := range r.Results {
		...
		drawTableRow(pdf, y, row, colW, colWhite, rowColor, 8.5)
		y += rowH
	}
	return y
}
...
func drawRiskTable(pdf *gopdf.GoPdf, y float64, r *ReportData) float64 {
	...
	for i := range r.Results {
		...
		drawTableRow(pdf, y, row, colW, colWhite, rowColor, 8)
		y += rowH
	}
	return y
}
...
// drawAdvice 结论与建议
func drawAdvice(pdf *gopdf.GoPdf, y float64, r *ReportData) float64 {
	var lines []string
	lines = append(lines,
