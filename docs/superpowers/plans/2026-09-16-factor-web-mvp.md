...
3. 逐个把 1～4 个区间条件加入预设买入策略，全部条件固定为 AND。
4. 由后端构造基准、各单条件和全部条件组合的 `core.Variant`（N≥2 时 N+2 个；
   N=1 时组合增强与单条件等价，共 2 个）。
...
- `factorFilters` 必须有 1～4 项；服务端不接受空数组或超过上限
  （2026-09-16 由 2～4 放宽）。
...
1. version、preset、1～4 项数量、重复 `kind+days`、factor kind、days、operator、有限数和区间边界校验。
2. 每项 `gte/lte/between` 正确映射到 `buy.A因子过滤.Min/Max`。
3. N≥2 时生成 N+2 个 Variant：基准、N 个单条件、一个组合增强；N=1 时只生成 基准+单条件 2 个。
...
2. 简单配置包含：预设策略、1～4 个因子条件、卖出规则、年份和样本。
...
1. 用户不改代码即可完成逐个因子理解、1～4 条 AND 组合和基准/组合增强回测。
